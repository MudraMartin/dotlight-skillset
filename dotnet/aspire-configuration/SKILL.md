...
See the `aspire-integration-testing` skill for patterns on passing
...
- `aspire-service-defaults` — shared OpenTelemetry / health checks / resilience setup
- `aspire-integration-testing` — `DistributedApplicationTestingBuilder` patterns
- `microsoft-extensions-configuration` — `IOptions<T>` binding and validation
