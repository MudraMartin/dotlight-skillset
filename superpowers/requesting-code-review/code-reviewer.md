...
Subagent (general-purpose):
  ...
  prompt: |
    ...
    **Reviewability:**
    - Report the `git diff --stat` output. If the diff exceeds ~400 changed lines for a single task, flag as Important: it exceeds a human review budget.
    - Does the diff introduce any interface/abstract class with exactly one implementation? Apply the deletion test — inline it unless a second implementation exists today.
    - Any options, hooks, configurability, or generality no current caller uses? Flag for removal.
    ...
    **Production readiness:**
    ...
    [Architecture problems, missing features, poor error handling, test gaps, unforced scope creep]
