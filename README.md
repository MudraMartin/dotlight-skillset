...
### .NET patterns (29 skills)

| Skill | Role |
|---|---|
| `nhibernate-patterns` 🆕 | **Dotlight original** — NH as first-class runtime (most dotlight projects are NHibernate): session-per-request DI, ISession vs IStatelessSession, dirty-checking writes (no `Update()` call), fetch strategies (batch-size/Fetch/Futures), HQL bulk + cache bypass, LINQ→QueryOver fallbacks, Npgsql jsonb/array/enum IUserTypes, FluentMigrator/DbUp schema evolution (+reference.md) |
